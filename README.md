...
| `?panel=0` | パネルを強制非表示 |
| `?panel=1` | パネルを強制表示（`obsMode` 設定や `?custom=...` より優先。OBSブラウザソース等で設定パネルが隠れて操作不能になった時の復旧用） |
...
**復旧方法**: OBS ブラウザソースの URL 末尾に `?panel=1` を付けてリロード（既に別パラメータが付いていたら `&panel=1`）。`?panel=1` は `obsMode` や `?custom=...` より優先してパネルを強制表示します。設定タブで「OBS用に起動時パネル非表示」を OFF に戻したら、URL から `?panel=1` を外して通常運用に戻してください。
